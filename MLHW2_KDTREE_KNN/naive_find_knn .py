	...
	for i in range(3):
		for data in train_data:
			...
			for j in range(1, DIMENSION+1):
				...
				dist  = dist + tmp*tmp
			if len(sys.argv) > 1 and K == [1, 5, 10, 100][int(int(sys.argv[1])/3)] and i == int(int(sys.argv[1])%3) and (train_data.index(data) == int(sys.argv[2]) or train_data.index(data) == int(sys.argv[3])):
				print('id =', train_data.index(data), ', ', dist)
